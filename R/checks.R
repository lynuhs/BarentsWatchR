...
#'
#' @export
barentswatch_check_existing_token <- function(token = BarentsWatchAuth$public_fields$token){

  cache_path <- app_id <- app_secret <- FALSE
  ...
  cache_path <- is.different(token$cache_path, "barentswatchR.httr_oauth_cache")

  if(!is.null(token$app)){
    app_id     <- is.different(token$app$key, "barentswatchR.app_id")
    app_secret <- is.different(token$app$secret, "barentswatchR.app_secret")
